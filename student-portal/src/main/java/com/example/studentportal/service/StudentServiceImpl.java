...
public class StudentServiceImpl implements StudentService {
    ...
    public List<Student> getAllStudents() { return new ArrayList<>(studentDb.values()); }
    public Optional<Student> getStudentById(Long id) { return Optional.ofNullable(studentDb.get(id)); }
    public Student saveStudent(Student student) {
        if (student.getId() == null) student.setId(idCounter.incrementAndGet());
        studentDb.put(student.getId(), student);
        return student;
    }
    public void deleteStudent(Long id) { studentDb.remove(id); }
}
